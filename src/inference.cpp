...
int Inference::inference() {
	...
	char fileName[200];
	strcpy(fileName, result_dir.c_str());
	strcat(fileName, "perplexity.txt");
	cout<<fileName<<endl;
	FILE * fout = fopen(fileName, "w");

	cout<<"Here in Perplexity"<<endl;
	if (!fout) {
...
//				int l = sentiLab;
//				int z = topic;
//				int w = pnewData->_pdocs[m]->words[n];
//				likelihood = likelihood + log(newphi_lzw[l][z][w]*newtheta_dlz[m][l][z]*newpi_dl[m][l]);
//			} // word loop
//			double tmp = pow(2.0, -likelihood/pnewData->_pdocs[m]->length / log2);
//			//cout<<"Doc = "<< m << "perplexity = " << tmp << endl;
//			perplexity = perplexity + tmp;
//		}// doc loop
//		cout<<"Perplexity at iteration:"<<liter<<" is :"<< perplexity/pnewData->numDocs;
//		//if (savestep > 0 && liter % savestep == 0) {
//		//	if (liter == niters) break;
//
//		if(liter>2)
//			avgPerplexity = avgPerplexity + perplexity/pnewData->numDocs;
//
//			printf("Saving the model at iteration %d ...\n", liter);
//			compute_newpi();
//			compute_newtheta();
...
//				int _w = pnewData->_pdocs[m]->words[n];
//
//
////				double piPart = new_nd[]
//				double sentiPart = (new_ndl[m][l] + gamma_l[l]) / (new_nd[m] + gammaSum);
//				double topicPart =(new_ndlz[m][l][z] + alpha_lz[l][z]) / (new_ndl[m][l] + alphaSum_l[l]);
//				double wordPart = (nlzw[l][z][w] + new_nlzw[l][z][_w] +  beta_lzw[l][z][_w])/(nlz[l][z]+new_nlz[l][z]+betaSum_lz[l][z]);
//
//
//				likelihood = likelihood + log(sentiPart*topicPart*wordPart);
//			} // word loop
//			docPerplexity = pow(2.0, -likelihood/pnewData->_pdocs[m]->length / log2);
//
//		}// iterate loop
//		cout<<" "<<docPerplexity<< endl;
//		perplexity = perplexity + docPerplexity;
//	}

	cout<<"Alpha is:"<<_alpha<<" gamma is:"<<_gamma<<endl;
    double docPerplexity = 0.0;
    perplexity=0.0;
	for (int m = 0; m < pnewData->numDocs; m++) {
		printf("Doc %d : ", m);
		...
		for (liter = 1; liter <= niters; liter++) {
			...
			for(int l=0; l<numSentiLabs; l++){
				for(int z=0; z<numTopics; z++){
					likelihood = likelihood + gamma_l[l]*log(newpi_dl[m][l])+alpha_lz[l][z]*log(newtheta_dlz[m][l][z]);
				}
			}
			...
			for (int n = 0; n < pnewData->_pdocs[m]->length; n++) {
				...
				double sentiPart = (new_ndl[m][l] + gamma_l[l]) / (new_nd[m] + gammaSum);
				double topicPart =(new_ndlz[m][l][z] + alpha_lz[l][z]) / (new_ndl[m][l] + alphaSum_l[l]);
				double wordPart = (nlzw[l][z][w] + beta_lzw[l][z][w] + beta_lzw[l][z][_w])/(nlz[l][z]+betaSum_lz[l][z]);

				likelihood = likelihood + log(sentiPart*topicPart*wordPart);
			} // word loop

		}// iterate loop
		likelihood = likelihood/niters; // average joint probability
		docPerplexity = pow(2.0, -likelihood/pnewData->_pdocs[m]->length / log2);
		cout<<" "<<docPerplexity<<", likelihood: "<<likelihood<< endl;
		perplexity = perplexity + docPerplexity;
	}

	cout<<"The number of docs:"<<pnewData->numDocs<<endl;
	fprintf(fout, "%s ", "The number of docs:");
	fprintf(fout, "%d ", pnewData->numDocs);

	perplexity = perplexity / pnewData->numDocs;
	cout<<"Perplexity is:" <<perplexity<<endl;

	fprintf(fout, "\n%s ", "Perplexity is:");
	fprintf(fout, "%lf ", perplexity);
	...
	compute_newphi();
	//save_model(model_name + "_" + putils->generate_model_name(-1));
	//cout<<"Average Perplexity is:"<< avgPerplexity/(niters-2);
	return 0;
}*/
...
void Inference::compute_newpi(int m) {
	for (int l = 0; l < numSentiLabs; l++) {
...
void Inference::compute_newtheta(int m) {
	for (int l = 0; l < numSentiLabs; l++)  {
		for (int z = 0; z < numTopics; z++) {
...
/*
double Inference::calculate_perplexity(){
       ...
       for(int m = 0; m < pnewData->numDocs; m++) {
       double val = 0.0;
            for (int r = 0; r < pnewData->_pdocs[m]->length; r++) {
                double prob = 0.0;
                int w = pnewData->_pdocs[m]->words[r];
                for (int l = 0; l < numSentiLabs; l++) {
	                for (int z = 0; z < numTopics; z++) {
	                		prob = prob + newphi_lzw[l][z][w]*newtheta_dlz[m][l][z]*newpi_dl[m][l];
                    	} // for z
                } // for l
                val = val + log(prob);
		}
        double tmp = pow(2.0, -val/pnewData->_pdocs[m]->length / log2);
        cout<<"Doc = "<< m << "perplexity = " << tmp << endl;
        perplexity = perplexity + tmp;
       }*/
...
/*
       ...
       for(int m = 0; m < pnewData->numDocs; m++) {
    	   double val = 0.0;
    	   for (int n = 0; n < pnewData->_pdocs[m]->length; n++) {
    		   // index of the word w inside the document //
    		   int w = pnewData->_pdocs[m]->words[n];
    		   int l = new_l[m][n];
    		   int z = new_z[m][n];
    		   if(l==-1 || z==-1) continue;
    		   val = val + log(newphi_lzw[l][z][w]*newtheta_dlz[m][l][z]*newpi_dl[m][l]);
    	   }
    	   double tmp = pow(2.0, -val/pnewData->_pdocs[m]->length / log2);
    	   cout<<"Doc = "<< m << "perplexity = " << tmp << endl;
    	   perplexity = perplexity + tmp;
...
double Inference::calculate_perplexity(){
       ...
       for(int m = 0; m < pnewData->numDocs; m++) {
    	   ...
    	   double tmp = pow(2.0, -val/pnewData->pdocs[m]->length / log2);
    	   cout<<"Doc = "<< m << "perplexity = " << tmp << endl;
    	   perplexity = perplexity + tmp;
       }
...
int Inference::max_three(int a, int b, int c)
{
    if(a>=b && a>=c)
        return 0;
    else if(b>=a && b>=c)
...
int Inference::calculate_precision_recall()
{
       
	char fileName[200];
	strcpy(fileName, result_dir.c_str());
	strcat(fileName, "predicted_label.dat");
	cout<<fileName<<endl;
       	//FILE * fout = fopen("./result/tv/predicted_label.dat", "w");
       	
	FILE * fout = fopen(fileName, "w");
       	
	cout<<"Here in Precison"<<endl;
        if (!fout) {
    	    printf("Cannot save file %s!\n", "./result/");
    	    return 1;
        }
       	...
       	char fileName1[200];
	strcpy(fileName1,result_dir.c_str());
	strcat(fileName1, "predicted_label_sentence.dat");
	cout<<fileName1<<endl;
        //FILE * fout_sen = fopen("./result/tv/predicted_label_sentence.dat", "w");
       
	FILE * fout_sen = fopen(fileName1, "w");
	...
	if (!fout_sen) {
       ...
       int *count;
       count = new int [pnewData->vocabSize];
       int sentence_label_count[3];
       
       
       for(int m = 0; m < pnewData->numDocs; m++) {
          
          sentence_label_count[0] = 0; // neutral
          sentence_label_count[1] = 0; // pos
          sentence_label_count[2] = 0; // neg
          ...
          for (int r = 0; r < pnewData->vocabSize; r++) {  
           ...
           //wordCount in the doc m
           for (int r = 0; r < pnewData->_pdocs[m]->length; r++) {  
               int index = pnewData->_pdocs[m]->words[r];
               count[index]++;
            ...
            for (int r = 0; r < pnewData->_pdocs[m]->length; r++) {  
                ...
                for (int l = 0; l < numSentiLabs; l++) {
	                for (int z = 0; z < numTopics; z++) {
                        exp =  count[w]*newphi_lzw[l][z][w]*newtheta_dlz[m][l][z]*newpi_dl[m][l];
                        if(exp>max)
					    {
    						max = exp;
    						predicted_label = l;
               ...
               //cout<<m<<" Word = "<<w<< " Label: "<<predicted_label<< "Count:" << count[w] <<endl; 
               fprintf(fout, "%d ", predicted_label);
               sentence_label_count[predicted_label]++;

            }
          
          fprintf(fout, "\n");
		}
        //cout<<"The vocan size = " <<pnewData->vocabSize << endl;   
       	fclose(fout); 
       	
       	
       	// sentence label prediction
       double word_topic[ pnewData->vocabSize][numSentiLabs][numTopics];
       for(int m = 0; m < pnewData->numDocs; m++) {
       int number_of_words = pnewData->_pdocs[m]->length;
         
          // initialize for each doc actually a sentence
          for(int a=0; a<pnewData->vocabSize; a++)
          {
            for(int b=0; b<numSentiLabs; b++)
            {
                for(int c=0; c<numTopics; c++)
                  {
                        word_topic[a][b][c] = 0.0;  
                  }
            }
          }
         ...
         for (int r = 0; r < pnewData->vocabSize; r++) {  
           ...
           cout<<"\nDoc id = " <<m<<endl;
           fprintf(fout_sen, "d%d ", m);
           
           //wordCount in the doc m
           for (int r = 0; r < pnewData->pdocs[m]->length; r++) {
               int index = pnewData->_pdocs[m]->words[r];
               count[index]++;
           }
